...
before.filter.box <- ggplot(df, aes(y = "", x = transcript.percent.expression)) +
  geom_boxplot() +
  labs(y = "", x = "Expression Percentage (%)", title = "BEFORE removing isoforms \nexpressed in =<1% of cells") +
  theme_classic() +
...
colnames(summary_table.before)[1] <- "Expression Frequency \nBEFORE Filtering (%)"
...
table1 <- data.frame(
  "Number of Transcript Removed" = num_removed_trans,
  "Total Counts Before Filtering" = sum(count_matrix),
  "Total Counts After Filtering" = sum(count_matrix.subset),
  "Number of Raw Counts Removed" = sum(count_matrix) - sum(count_matrix.subset)
) %>% 
  t(.)
...
colnames(summary_table.after)[1] <- "Expression Frequency \nAFTER Filtering (%)"
...
ggsave(pdfFilename, concatenated.layout, width = 10, height = 25)
