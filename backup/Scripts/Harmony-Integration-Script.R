...
Iso.density.plt.LR.after.filter <- compareFeaturesDetected(long.reads.IsoLvl.combined, "Long Reads", "Isoforms")
...
cell.num.bar.plt.LR.FILTERED | cell.num.bar.plt.SR.FILTERED
...
before.harmony.shortReads <- DimPlot(short.reads.combined.filtered, reduction = 'umap', group.by = 'orig.ident') +
  labs(title = "Short Read Day 25 and 55 Samples\nBefore Integration")
...
long.reads.combined.filtered <- RunUMAP(long.reads.combined.filtered, dims = 1:11, reduction = 'pca') # Choosing 12 PCs based on elbow plot

before.harmony.longReads <- DimPlot(long.reads.combined.filtered, reduction = 'umap', group.by = 'orig.ident') +
  labs(title = "Long Read Day 25 and 55 Samples\nBefore Integration")
before.harmony.longReads

before.harmony.longReads | before.harmony.shortReads
...
short.reads.harmony$seurat_clusters <- short.reads.harmony$"RNA_snn_res.0.5"
...
SR.cluster_resolution.figs[[1]] | SR.cluster_resolution.figs[[2]] | SR.cluster_resolution.figs[[3]] |
  SR.cluster_resolution.figs[[4]] | SR.cluster_resolution.figs[[5]]

UMAP.integrated.short.reads <- DimPlot(short.reads.harmony, reduction = 'umap', group.by = 'orig.ident') +
  labs(title = "Short Read Day 25 and 55 Samples\nAfter Integration")
UMAP.integrated.short.reads | SR.cluster_resolution.figs[[3]]
...
UMAP.integrated.long.reads <- DimPlot(long.reads.harmony, reduction = 'umap', group.by = 'orig.ident') +
  labs(title = "Long Read Day 25 and 55 Samples\nAfter Integration")
UMAP.integrated.long.reads | LR.cluster_resolution.figs[[2]]
...
LR.cluster_resolution.figs[[1]] | LR.cluster_resolution.figs[[2]] | LR.cluster_resolution.figs[[3]] |
  LR.cluster_resolution.figs[[4]] | LR.cluster_resolution.figs[[5]]
...
clusters.long.reads.IsoLvl.beforeTransfer <- DimPlot(long.reads.harmony.IsoLvl,
                                                     reduction = 'umap',
                                                     group.by = "RNA_snn_res.0.7", label = TRUE,
...
clusters.long.reads.IsoLvl.beforeTransfer
...
DimPlot(short.reads.harmony, dims = c(1,2), reduction = 'harmony', group.by = "orig.ident")
DimPlot(long.reads.harmony, dims = c(1,2), reduction = 'harmony', group.by = 'orig.ident')
...
sr.CellPhase.UMAP | UMAP.integrated.short.reads
lr.CellPhase.UMAP | UMAP.integrated.long.reads
lr.CellPhase.UMAP | pc1.pc2.lr.cellphase.plt
sr.CellPhase.UMAP | pc1.pc2.sr.cellphase.plt
pc1.pc2.sr.cellphase.plt | pc1.pc2.lr.cellphase.plt
...
pdf("Harmony_Integration_UMAPs.pdf", width = 11, height = 8)
...
#grid.draw(before.harmony.longReads)
#grid.draw(UMAP.integrated.long.reads)
...
pdf("Metadata-SR.LR.comparison.pdf", width = 22, height = 8)
grid.draw(LR.UMI.Gene.Association.plt.BEFORE.FILTER | LR.UMI.Gene.Association.plt.AFTER.FILTER)
grid.draw(SR.UMI.Gene.Association.plt.BEFORE.FILTER | SR.UMI.Gene.Association.plt.AFTER.FILTER)
grid.draw(LR.UMI.Gene.Association.plt.AFTER.FILTER | SR.UMI.Gene.Association.plt.AFTER.FILTER)

grid.draw(UMI.density.plt.SR | UMI.density.plt.SR.AfterFilter)
grid.draw(UMI.density.plt.LR | UMI.density.plt.LR.AfterFilter)

grid.draw(Gene.density.plt.SR | Gene.density.plt.SR.AfterFilter)
grid.draw(Gene.density.plt.LR | Gene.density.plt.LR.AfterFilter)
grid.draw(Iso.density.plt.LR | Iso.density.plt.LR.after.filter)

grid.draw(cell.num.bar.plt.LR | cell.num.bar.plt.LR.FILTERED)
grid.draw(cell.num.bar.plt.SR | cell.num.bar.plt.SR.FILTERED)
grid.draw(cell.num.bar.plt.LR.FILTERED | cell.num.bar.plt.SR.FILTERED)
# Close the PDF device
dev.off()


### Save our cell cycle figures for short reads and long reads into a PDF-------------------------------
pdf("Effects-Of_CellCycle-v2.pdf", width = 22, height = 8)

grid.draw(sr.CellPhase.UMAP | UMAP.integrated.short.reads)
grid.draw(lr.CellPhase.UMAP | UMAP.integrated.long.reads)
grid.draw(lr.CellPhase.UMAP | pc1.pc2.lr.cellphase.plt)
grid.draw(sr.CellPhase.UMAP | pc1.pc2.sr.cellphase.plt)
grid.draw(pc1.pc2.sr.cellphase.plt | pc1.pc2.lr.cellphase.plt)
