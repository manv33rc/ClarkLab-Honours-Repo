...
# Create a transcript-gene ID key
...
filename <- paste("filt_", output_prefix, "_transcript_count_WITHGENEID.csv", sep = "")
#write.csv(count_matrix.subset, filename)

matched_gene_ids <- transcript_gene_key$gene_id[match(row.names(count_matrix.subset), transcript_gene_key$transcript_id)]
count_matrix.subset$gene_id <- matched_gene_ids
View(count_matrix.subset)
which(colnames(count_matrix.subset) == "gene_id")
#count_matrix.subset_copy <- count_matrix.subset
#row.names(count_matrix.subset) <- matched_gene_ids